    ...
    <div class="cards-actualite d-flex flex-wrap justify-content-around p-3">
        ...
        <div class="card-actualite w-25 mx-1 border-0">
            ...
            <div class="card-body my-1 p-3">
                <a class="text-uppercase font-weight-normal text-dark" href="<?= $router->generate('post_page', ['slug' => $post->slug, 'id' => $post->id]) ?>"><?= $post->title ?></a> <!-- <span class="font-weight-lighter">text</span> -->
                ...
                <a href="<?= $router->generate('post_page', ['slug' => $post->slug, 'id' => $post->id]) ?>" class="btn btn-dark rounded-0 mt-1 mb-4 ml-auto font-weight-lighter">En savoir plus</a>
            </div>
        </div>
        <?php endforeach ?> 
    </div>
